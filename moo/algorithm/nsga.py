...
class NSGAII(Algorithm):
    ...
    def _evaluate(self, population: torch.Tensor) -> torch.Tensor:
        """
        population: shape: (pop_size, n_obj)
        """
        targets = []
        for fn in self.target_fn:
            targets.append(fn(population))
    ...
    def _mutation(self, parents: torch.Tensor) -> torch.Tensor:
        """
        target: shape: (pop_size)
        """
        shape = parents.shape
        mutation_prob = torch.rand(shape, device=self.device) < 1
        mutation = (
...
if __name__ == '__main__':
    ...
    nsga = NSGAII(target_fn=[fn1, fn2], bounds=[(-55, 55)], pop_size=3000)
